...
This tutorial requires for the libraries `magrittr` for its pipe-like operator, `dplyr`, `jaccard` and `ggplot2`. We also automatically load the `doParallel` and `foreach` libraries.
`jaccard`
